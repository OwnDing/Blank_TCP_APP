...
namespace Blank_TCP_Server.Methods
{
    ...
    internal sealed class Token : IDisposable
    {
        ...
        internal void ProcessData(SocketAsyncEventArgs args)
        {
            ...
            Console.WriteLine("Received: \"{0}\". The server has read {1} bytes from client:{2}", received, received.Length, connection.RemoteEndPoint.ToString());
            //String msg="Received: "+received+". The server has read "+received.Length.ToString()+" bytes from client:"+connection.RemoteEndPoint.ToString();
            //Byte[] sendBuffer = Encoding.ASCII.GetBytes(received);
            //Console.WriteLine("bytes from client:{0}", BitConverter.ToString(sendBuffer).TrimEnd('\0'));
        ...
        internal void SetData(SocketAsyncEventArgs args)
        {
            ...
            //sb.Append(BitConverter.ToString(args.Buffer, args.Offset, count).TrimEnd('\0'));

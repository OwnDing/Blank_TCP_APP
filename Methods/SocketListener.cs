...
namespace Blank_TCP_Server.Methods
{
    ...
    internal sealed class SocketListener
    {
        ...
        private void CloseClientSocket(Token token, SocketAsyncEventArgs e)
        {
            ...
            //close Socket,remove the not connected socket
            closeSocket();
            ...
            if (s.Connected)
            {
                try
                {
                    ...
                    if (readEventArgs != null)
                    {
                        ...
                        lock (socketDic)
                        {
                            if (socketDic.ContainsKey(s.RemoteEndPoint.ToString()))
                            {
                                socketDic.Remove(s.RemoteEndPoint.ToString());
                                if (eventlistview != null)
                                    eventlistview(s.RemoteEndPoint.ToString(), "D");
                            }
                            socketDic.Add(s.RemoteEndPoint.ToString(), s);
                            if (eventlistview != null)
                                eventlistview(s.RemoteEndPoint.ToString(), "C");
                        }
            ...
            if (e.BytesTransferred > 0)
            {
                if (e.SocketError == SocketError.Success)
                {
                    ...
                    if (s.Available == 0)
                    {
                        // Set return buffer.
                        token.ProcessData(e);
                        if (true) //(!s.SendAsync(e))  skip the send progress
                        {
        ...
        public void show_count()
        {
            lock (socketDic)
            {
                Console.WriteLine("connections number:" + socketDic.Count.ToString());
            }
        }
        private void closeSocket()
        {
            ...
            lock (socketDic)
            {
                foreach (KeyValuePair<string, Socket> kvp in socketDic)
                {
                    if (!kvp.Value.Connected)
                    {
                        Console.WriteLine("loop the socket array!remove not connected socket");
                        key = kvp.Key;
                    }
                }
                if (key != string.Empty)
        ...
        public void Send2client(Socket server, String data)
        {
